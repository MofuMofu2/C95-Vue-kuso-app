...
私は「クソアプリカレンダー面白そう！枠も空いている！よーしポチッとな！」と参加登録をした後で大変なことに気づきました。@<strong>{アプリケーションを作って公開したことがないのです}。なんでもそうですが、毎回申し込んだ後に「やめておけばよかった」と後悔しつつ何とか締め切りに合わせて完成させる、みたいな生活なはほんと改善したいです。
...
ここでの@<tt>{本格的なもの}とは、次の要素を含んでいるWebアプリケーションを指します。
...
開発者ツール的なものは始めから対象から外しました。そもそも@<strong>{KUSO}アプリケーションを作成するのですから、開発者向けツールとは全く相性が合いません。主旨を外れてしまいます。
...
@<tt>{動的な}状態とは、クライアントのリクエストに応じてWebサイトの内容が書き換わるWebアプリケーションのことを指しています。
@<tt>{静的な}状態はこの逆で、一度アクセスするとWebサイトの内容はずっと同じ状態です。
...
これらの要素から、@<strong>{同人誌のポートフォリオサイト}をVue.jsで作ろうという結論に達しました。
...
項目が決まったので、レイアウトの検討に移ります。@<tt>{ポートフォリオサイトまとめ}といったタイトルがついているWebページを開いては「オシャンティすぎる…」と肩を落とすことを20回くらい繰り返しました。
...
@<tt>{ワイヤーフレーム}は、Webサイトを構成する要素の配置図の役割を持っています。絵を描く人のメイキングを見ると、
...
これはWebサイトのデザインも一緒で、ワイヤーフレームを作成してデザインの配置を決めたあと、@<tt>{デザインカンプ}と言われる完成品の絵を描き上げ、それを元にコーディングを行うのが一般的な制作フローとなっています。
...
//image[photoshop][PhotoShopで細かい色やフォントサイズを指定する]{
//}
