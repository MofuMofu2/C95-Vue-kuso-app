...
こんにちは。りまりま団のもふもふです。この本はノリで@<href>{https://qiita.com/advent-calendar/2018/kuso-app,クソアプリ Advent Calendar2018} （@<tt>{https://qiita.com/advent-calendar/2018/kuso-app}）に申し込んでしまったもふもふちゃんがVue.jsでKUSO同人誌ポートフォリオをつくった記録をまとめたものです。


== 使った技術スタック


このWebアプリケーションを作成するために使ったモノを@<strong>{全部}書きます。この本はお役に立てそうでしょうか？
...
普段ブラウザはChromeを利用しているので、デバッグもChromeで行いました。Vue.js公式から@<href>{https://chrome.google.com/webstore/detail/vuejs-devtools/nhdogjmejiglipccpnnnanhbledajbpd?hl=ja,デバッグツール}が提供されているため、データがきちんと割り当てされているかはChromeで確認しました。



処理のデバッグ？それは@<tt>{console}メソッドを使って行いました。VS Codeのデバッグツールは私には早すぎました。
...
@<href>{https://github.com/MofuMofu2/portfolio-vue,portfolio-vue}という名前でGitHub上にリポジトリを作成しました。
...
GitHub上のissue順に何を考えて作ったかを書きました。Vue.jsの書き方などの解説はせず、どのように考えて試行錯誤したかを中心に記載しています。ドキュメントや本・ブログを見ても「文法や機能はわかった。でも結局どーやって作り始めたらいいのかわからーん！」と思ってしまうので、私はこのように進めました。という知見はそれなりに有用なのではないか？と思うからです。
...
加えて、紙面の都合上コードの途中で改行している箇所があります。改行箇所に@<tt>{\r}を記載している場合、ここで改行されたよという意味です。参考にする場合は改行・スペースを入れずに記載してください。
