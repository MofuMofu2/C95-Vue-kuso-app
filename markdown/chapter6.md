...
![ボタンを追加した](../images/chapter6/add_button.png)
...
原因は``vue.runtime.esm.js?2b0e:587 [Vue warn]: Error in render: "TypeError: Cannot read property 'book_title' of undefined"``の部分が示しているようです。JSONの中の``book_title``の値を特定できない、と推測することができます。
実際、同人誌は6種類しか存在しないので、7冊目を選択しようとしてもデータは存在しません。ですから**0以上6以下の範囲であれば**インデックスの加算・減算処理を行うようにするべきです。
...
この処理だと、``bookId``が配列の長さ（＝ 6）以外のときはbookIdに1を加算する処理となります。つまり、**bookIdが6**のときは加算処理がされず、それ以上の数は永遠に``bookId``の中に代入されない＝**6以上のインデックスは**指定されないようになります。
しかし、よく考えてください。**配列のインデックスは0から始まります**。つまり、配列のデータ数が**6**であれば、インデックスの最大値は**5**です。今の処理は**6**が``bookId``に代入されます。しかし、実際は7番目のデータを取得する処理となります。これではデータを探せるはずがありません。
...
まず、bookIdが``配列の長さ - 1``以下か判定します。``配列の長さ - 1``以下であれば、``bookId``に1を加算します。これにより、同人誌の次の情報を取得できますね。次に、``else..if文``を使って``配列の長さ - 1以下ではなかった場合``、``bookId``が``配列の長さ - 1``以上（＝ 5よりも大きい）になったかを調べます。``if``分岐の処理で``bookId``はどうしても``6``になってしまうためです。その場合は加算処理を行わず、配列の長さと同じ値を``bookId``へ代入し返却します。こうすることで、配列の長さの最大値をインデックスとして指定できるようになります。
...
まっっっっっっっっっっっっっっっっっっったく同じことを思いました。が、このエラーが出ているときは〇〇の処理の1つ前の処理で``undefined``になってしまっていることがほとんどです。1つ前の処理が正しくできているか？を標準出力/デバッガーを使って確認しましょう。
